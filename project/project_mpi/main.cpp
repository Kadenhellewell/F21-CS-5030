...
Point rungeKutta(Point p, float x_step);
...
//    MPI_Comm_rank(comm, &my_rank);
    ...
    {
        int lineId = row;
        for(int col = 0; col < 10; col++)//this loop doesn't need to depend on columns, some lines loop in vortex. pick stopping time
        {
            ...
            cout << lineId << ", " << current.x_coord << ", " << current.y_coord << endl;
        }
    }
...
Point rungeKutta(Point p, float x_step)
{
    ...
    k1 = const_vect_mult(x_step, get_v_from_field(p));
    ...
    return add_vector_point(p, const_vect_mult(0.1667, tempSum));
}
//Algorithm from: https://web.cs.ucdavis.edu/~ma/ECS177/papers/particle_tracing.pdf

bool not_in_range(Point p)
{
    if(p.x_coord < 0 || p.x_coord >= data_cols || p.y_coord < 0 || p.y_coord >= data_rows)
        return true;
    return false;
}
